...
def get_df(fpath):
    def arg2df(fpath):
        ...
        with open(fpath, 'r') as f:
            ...
            columns = [ele.split('=')[0] for ele in lines[0][:-1]]
            print(lines)
            # [print(ele) if len(ele.split('=')) < 2 else 0 for line in lines for ele in line ]
            rows = [[ele.split('=')[1] for ele in line[:-1]] for line in lines]
...
def diversity(df, fname = '../figure/tabu/test.png'):
    ...
    y = df[col_y].to_numpy() / 100 / 529
    y[0] = np.average(y[1:])
    ax1.set_xlabel('Generation')
    ax1.set_ylabel('Diversity')
    ax1.plot(x, y)

    lin = np.linspace(np.min(y), np.max(y), 30)
    y2 = np.array([(lin[i + 1] + lin[i]) / 2 for i in range(len(lin) - 1)])
    x2 = np.array([
        np.count_nonzero(np.logical_and(y >= lin[i], y <= lin[i + 1])) for i in range(len(lin) - 1)
    ])
    x2 = 50000 - x2 * 300
    # ax2 = ax1.twinx()
    # ax2.set_xlim([0, 200])
    ax1.plot(x2, y2, color=colors[1])
    print(x2)
    print(y2)
...
def best_max_min_avg(df, fname):
    ...
    for col, clr_idx, lb in zip(col_ys, clr_idxs, lbs):
        ...
        y[0] = y[1]
        # print(y)
        ax1.plot(x, y, color=colors[clr_idx])
    # ax1.legend()
...
def new_offspring(df, fname):
    ...
    fig, ax1 = plt.subplots()
    ax1.set_xlabel('Generation')
    ax1.set_ylabel('Survived Offspring Ratio(%)')

    y = df['new'].to_numpy()[1:] / 80 / 50 * 100
    ax1.plot(x, y)
    plt.savefig(fname)
...
#     y = df['hit'].to_numpy() / (50 * 80) * 100
#     ys.append(np.array(fitted_order(y, 10000)[st: ed]))
#     # plot(ax1, x, y, 'Tabu Hit', 10000)

#     y = df['wams_miss'].to_numpy() / (50 * 80) * 100
...
def main():
    ...
    # new_offspring(df, pfx + 'new_offspring.png')
